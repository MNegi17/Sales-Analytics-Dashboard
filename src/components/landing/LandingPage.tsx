...
import { Upload, BarChart3, ShieldCheck, Zap, Table, Calendar, Sparkles, ArrowRight, Layers, CheckCircle2, Lock, LucideIcon } from 'lucide-react';
      ...
      <section className="relative overflow-hidden bg-white rounded-3xl p-6 sm:p-10 lg:p-12 border border-slate-200 shadow-sm text-center">
        ...
        <div className="relative max-w-4xl mx-auto space-y-6">
          ...
          <p className="max-w-2xl mx-auto text-sm sm:text-base lg:text-lg text-slate-600 font-normal leading-relaxed">
            Upload ONE raw sales report file to automatically aggregate, process, and generate identical Excel sheets across all 7 marketplaces.
          </p>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3.5 pt-4">
            ...
            >
              ...
              <span>Upload Sales File Now</span>
            </button>

            <button
              onClick={onViewDashboard}
              className="w-full sm:w-auto px-7 py-3.5 rounded-xl bg-slate-100 hover:bg-slate-200 active:scale-98 text-slate-800 font-bold text-xs sm:text-sm border border-slate-300 transition-all flex items-center justify-center space-x-2"
            >
      ...
      <section className="bg-white p-6 sm:p-8 rounded-2xl border border-slate-200 shadow-xs space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
          <div className="p-5 rounded-xl bg-slate-50 border border-slate-200 space-y-3 relative">
            ...
            <h3 className="font-bold text-sm text-slate-900">Upload Sales File</h3>
            <p className="text-xs text-slate-600 leading-relaxed">
              Drag &amp; drop your raw sales orders Excel file containing date, channel name, category, and sales quantities.
            </p>
          </div>
          ...
          <div className="p-5 rounded-xl bg-slate-50 border border-slate-200 space-y-3 relative">
            ...
            <h3 className="font-bold text-sm text-slate-900">Automated Ingestion</h3>
            <p className="text-xs text-slate-600 leading-relaxed">
              The engine validates rows, maps channels to marketplaces, calculates daily totals, and resolves duplicate entries.
            </p>
          </div>
          ...
          <div className="p-5 rounded-xl bg-slate-50 border border-slate-200 space-y-3 relative">
            ...
            <h3 className="font-bold text-sm text-slate-900">View &amp; Export Excel</h3>
            <p className="text-xs text-slate-600 leading-relaxed">
              Explore exact spreadsheet layouts with frozen panes, toggle sub-channel totals, and download complete `.xlsx` workbooks.
            </p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-gradient-to-r from-slate-900 to-sky-950 text-white rounded-2xl p-6 sm:p-8 flex flex-col sm:flex-row items-center justify-between gap-4 shadow-md">
        <div className="space-y-1 text-center sm:text-left">
          <h3 className="font-bold text-base text-white flex items-center justify-center sm:justify-start space-x-2">
            ...
            <span>Ready to process your sales data?</span>
          </h3>
          <p className="text-xs text-slate-300">Start uploading files or browse existing aggregated analytics.</p>
        </div>
        ...
        <div className="flex items-center space-x-3 w-full sm:w-auto justify-center">
          ...
          <button
            onClick={onViewDashboard}
            className="px-5 py-2.5 rounded-lg bg-white/10 hover:bg-white/20 text-white font-bold text-xs border border-white/20 transition-all"
          >
            View Dashboard
          </button>
        </div>
